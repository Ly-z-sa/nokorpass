...
const css = `
  ...
  .footer-grid { display: grid; grid-template-columns: 1.2fr 1.5fr 1.5fr; gap: 40px; margin-bottom: 48px; }
  ...
  .footer-brand .logo { font-size: 1.2rem; margin-bottom: 15px; display: flex; align-items: center; gap: 8px; color: var(--text); text-decoration: none; font-weight: 600; }
...
const js = `
  ...
  window.handleDeleteAccount = async () => {
    const ok = await confirmDialog({
      ...
      message: 'This action is permanent. Your profile and all saved tickets will be removed and cannot be recovered.',
...
<head>
    ...
    <title>Account Settings — NokorPass</title>
...
fs.writeFileSync(path.resolve(__dirname, '../../account.html'), html);
